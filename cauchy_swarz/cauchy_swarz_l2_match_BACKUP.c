...
/*void box_corr_given_indices(const double* I, int img_r,int img_col, const int* box_arr,int box_num, const double*
  w_arr,int n_p,int m_p,const int* tpi,const int* tpj , int ind_num, double* C);*/

void cauchy_swarz_l2_match(const double* img,int img_r,int img_c, const int* box_arr,int box_num,const double* w_arr,double threshold,double* reconst_pat,int n_p,int m_p, double* L);
		 ...
		 )
{
    ...
    const double* reconst_pat = mxGetPr(prhs[4]); 
    int n_p = mxGetM(prhs[4]); 
    int m_p = mxGetN(prhs[4]); 

    double* reconst_pat_copy = mxMalloc(sizeof(double)*n_p*m_p);
    ...
    cauchy_swarz_l2_match(I,img_r,img_c,box_arr,box_num, w_arr, threshold, reconst_pat_copy,n_p,m_p, mxGetPr(plhs[0]));
    /*    mxFree(box_arr_copy);*/
...
void cauchy_swarz_l2_match(const double* img,int img_r,int img_c, const int* box_arr,int box_num,const double* w_arr,double threshold,double* reconst_pat,int n_p,int m_p, double* L){
    ...
    printf("0\n");
    double* I = mxMalloc(sizeof(double)*(img_r+1)*(img_c+1));
    double* img_sqrs =  mxMalloc(img_r*img_c*sizeof(double));
    ...
    double* wins_sum_sqrs=mxMalloc(sizeof(double)*c_r*c_c);
    double* wins_norms=mxMalloc(sizeof(double)*c_r*c_c);
    double* C=mxMalloc(sizeof(double)*c_r*c_c);
    ...
    /*box_corr(const double* img,int img_r,int img_c, const int* box_arr,int box_num, const double* w_arr,int n_p,int m_p, double* C)*/
    ...
    double pat_sum_sqrs =sum_sqrs(reconst_pat,n_p,m_p);

     
   /*void matrix_add(const double* m1, const double* m2,double addScalar, double mulScalar1, double mulScalar2, int size, double* out){*/
    /*
    ## equivalent to:
    ...
    */
    ...
    /*double* L=mxMalloc(sizeof(double)*c_r*c_c);*/

    double mul2=(-1)*2*sqrt(pat_sum_sqrs);
    ...
    while(t<box_num){
        ...
        double mul=-2*sqrt(sum_sqrs(reconst_pat,n_p,m_p));
