function [  ] = test_ncc_match_with_cauchy_schwarz(points,filters_boxes1,l1,filters_boxes2,l2, debug,threshold)
...
%         
        ...
        boxes1=filters_boxes1(i,k).boxes;
        box_arr1=boxes1(:,1:4);
        w_arr1=boxes1(:,5);
        
        
           
        %rf=reconstruct_filter(box_arr,w_arr,n_p,m_p,0);
        %imshow(mat2gray(rf));
        %input('')
        %t0=tic;
        %[Lm,ind_left,ith,jth]=l2_match_with_cauchy_schwarz_bound1(single(I),pattern,n_p,m_p,box_arr,single(w_arr),single(threshold) );
        [Lm1,ind_left1,ith1,jth1,Lr1,Lt1]=ncc_match_with_cauchy_schwarz_bound1(I,pattern-sum(pattern(:))/(numel(pattern)),n_p,m_p,box_arr1(2:end,:),w_arr1(2:end),threshold,tl);
       ...
       % figure(20)
        ...
        box_arr2=boxes2(:,1:4);
        w_arr2=boxes2(:,5);
        [Lm2,ind_left2,ith2,jth2,Lr2,Lt2]=ncc_match_with_cauchy_schwarz_bound1(I,pattern-sum(pattern(:))/(numel(pattern)),n_p,m_p,box_arr2(2:end,:),w_arr2(2:end),threshold,tl);
        ...
        hold on
        plot(1:length(Lt2),threshold*ones(1,length(Lt2)),'--','LineWidth',2);
        ...
        disp('done 2');
         figure(20)
        %plot(1:length(percent_from_before),percent_from_before);
        l=min([length(ind_left1),length(ind_left2)]);
        
        
        plot(1:l,[ind_left1(1:l) ; ind_left2(1:l)]/numel(Lm1));
        axis([1 l 0 1])
       legend(l1,l2)
        
       figure(21)
       plot(1:length(w_arr1(2:8)),[w_arr1(2:8)' ; w_arr2(2:8)']);
       title('coefficients')
       legend(l1,l2)
       
       figure(345)
       imshow(plot_boxes(box_arr1(2:11,:),w_arr1(2:11),n_p,m_p));
%         bnum=min([7,length(localidx)]);
         %rf1=reconstruct_filter(box_arr1(localidx(1:bnum),:),w_arr1(localidx(1:bnum),:),n_p,m_p,0);
         rf1=reconstruct_filter(box_arr1,w_arr1,n_p,m_p,0);
         rf2=reconstruct_filter(box_arr2,w_arr2,n_p,m_p,0);
%         figure(97)
%         imshow(mat2gray(rf))
        if debug
            figure(995), imshow([pattern,rf1,rf2]), title('the pattern')
        end
         figure(30)
         imagesc([Lm1,Lm2])
        input('')
